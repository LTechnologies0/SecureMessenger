...
class EncryptedCredentialStore @Inject constructor(
    ...
    fun getProtocol(accountId: String): String? {
        appLockManager.assertUnlocked()
        return prefs.getString(prefKey(accountId, META_PROTOCOL), null)
    }

    /** Returns the stored display name for [accountId], or `null` if unknown. */
    fun getDisplayName(accountId: String): String? {
        appLockManager.assertUnlocked()
        return prefs.getString(prefKey(accountId, META_DISPLAY_NAME), null)
    }
    ...
    fun get(accountId: String, key: String): String? {
        appLockManager.assertUnlocked()
        return prefs.getString(prefKey(accountId, key), null)
    }
    ...
    fun getProxyPassword(): String? {
        appLockManager.assertUnlocked()
        return prefs.getString(PROXY_PASSWORD_KEY, null)
    }
}
